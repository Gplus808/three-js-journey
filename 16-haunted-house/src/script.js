...
gui.add(ambientLight, 'intensity').min(0).max(0.01).step(0.001)
scene.add(ambientLight)

// Directional light
const moonLight = new THREE.DirectionalLight('#ffffff', 0.1)
moonLight.position.set(4, 5, - 2)
gui.add(moonLight, 'intensity').min(0).max(0.1).step(0.001)
...
const tick = () =>
{
    ...
    ghost3.position.x = Math.cos(ghost3Angle) * 4 + Math.sin(elapsedTime * 3)
    ghost3.position.y = Math.sin(ghost3Angle) * 4 + Math.sin(elapsedTime * 4.7)
